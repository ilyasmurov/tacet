...
Run: `pnpm build && pnpm svg && pnpm meta && (cd site && node build.ts)`, then `python3 -m http.server 8765 --directory site/dist`, and open `http://localhost:8765/#gallery`. Search `latin-`; switch between 24 and 48 px and the variants A, B, C, D. (Do not judge the letters by `gallery.html` from `pnpm gallery`: it draws with a renderer of its own that knows neither accent spans nor dots sized by the stroke, so most accents are missing there.) Expected: soft capitals of one height, every letter shows its cut; in C and D one element per letter is in the accent colour. Screenshot 24 and 48 px in D and attach both to TACET-46.
...
Run: `pnpm build && pnpm svg && pnpm meta && (cd site && node build.ts)`, then `python3 -m http.server 8765 --directory site/dist`, and open `http://localhost:8765/#gallery`. Search `cyrillic-`; switch between 16, 24 and 48 px and the variants A, B, C, D. (Do not judge the letters by `gallery.html` from `pnpm gallery`: it draws with a renderer of its own that knows neither accent spans nor dots sized by the stroke, so most accents are missing there.) Expected: the twins look exactly like their Latin letters; З is clearly not a 3 and its tongue is the accent; Ц and Щ carry it on the tail, Й on the breve; the dots of Ё keep the weight of the stroke at every size. Screenshot 24 and 48 px in D and attach both to TACET-47.
...
Run: `pnpm build && pnpm svg && pnpm meta && (cd site && node build.ts)`, then `python3 -m http.server 8765 --directory site/dist`, and open `http://localhost:8765/#gallery`. Search `mark-`; switch between 24 and 48 px and the variants A, B, C, D. (Do not judge the letters by `gallery.html` from `pnpm gallery`: it draws with a renderer of its own that knows neither accent spans nor dots sized by the stroke, so most accents are missing there.) Expected: the marks sit where the paragraph above says; only ! and ? have an accent in C and D. Screenshot 24 and 48 px in D and attach both to TACET-48.
...
On `main` build the site as in Task 1 («Look at them») and open its gallery. For each of `bold`, `underline`, `strikethrough`, `subscript`, `superscript`, `translate` take a screenshot at 16, 24 and 48 px in variant D. Keep them as «before». (Not `gallery.html`: it does not draw accent spans, and the new B carries one.)
...
Build the site on the branch and take the same screenshots. Attach «before» and «after» side by side to TACET-56. Do not merge until Ilya has looked and agreed.
...
On a phone the menu now wraps onto a second line: with «Letters» six links and the theme toggle no longer fit one line of 375 px, and the toggle pushed the page sideways.
...
index 725c295..604b072 100644
--- a/site/styles.css
+++ b/site/styles.css
@@ -320,7 +320,9 @@ footer a:hover { color: var(--ink); }
   /* Navigation moves under the logo: it does not fit on one line, and the last
      links were cut off by the edge of the screen. */
   .top .inner { flex-wrap: wrap; gap: 12px 16px; }
-  .top nav { margin-left: 0; width: 100%; gap: 16px; }
+  /* Six links and the theme toggle do not fit one line of a phone: the toggle
+     pushed the page sideways. They wrap onto a second line instead. */
+  .top nav { margin-left: 0; width: 100%; gap: 8px 16px; flex-wrap: wrap; }
   .top nav a { font-size: 14px; }
   .hero { padding-block: 34px 10px; }
   .hero p { font-size: 18px; }
@@ -406,6 +408,35 @@ h2 .anchor { color: inherit; text-decoration: none; }
...
- at 375 px the page does not scroll sideways: `document.documentElement.scrollWidth <= document.documentElement.clientWidth` in the console is `true`. (Not `=== innerWidth`: when something sticks out, a phone widens the layout viewport and `innerWidth` grows with the page, so that comparison passes anyway);
- at 375 px the menu takes two lines, the theme toggle on the second, and nothing sticks out;
