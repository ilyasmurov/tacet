// Glyph variants that did not make it into the set.
//
// Fifty names existed in both source sets, and in fourteen of them the geometry
// had drifted apart. The owner picked a version by eye from a side-by-side
// comparison; the losing ones live here — if a familiar shape turns out to be
// missing while migrating a project, swapping it back is a one-line move.
//
// Kept in scripts rather than in the package: this is a reference for migrating
// projects, useless at runtime and with no business being in npm.
...
// arrow-left: the set uses the Taskless version; the Ansamblist variant is kept here
  "arrow-left": [p("M19 12H5", [[30, 18]]), p("M11 6l-6 6 6 6")],
  // check: the set uses the Taskless version; the Ansamblist variant is kept here
  "check": [p("M20 6 9 17l-5-5", [[42, 14]])],
  // edit: the set uses the Taskless version; the Ansamblist variant is kept here
  ...
  // eye-off: the set uses the Taskless version; the Ansamblist variant is kept here
  "eye-off": [p("M2.5 12S6 5.5 12 5.5c1.6 0 3 .4 4.3 1.1M21.5 12S18 18.5 12 18.5c-1.6 0-3-.4-4.3-1"), p("M4 20 20 4", [[42, 16]], { accent: true })],
  // filter: the set uses the Taskless version; the Ansamblist variant is kept here
  "filter": [p("M3 5h18", [[58, 16]]), p("M6 12h12", [[16, 16]]), p("M10 19h4", [[58, 22]])],
  // image: the set uses the Taskless version; the Ansamblist variant is kept here
  "image": [rc(3, 3, 18, 18, 2.5, [[13, 9], [63, 9]]), c(9, 9, 2, null, { accent: true }), p("m21 15-5-5L5 21", [[40, 14]])],
  // mention: the set uses the Ansamblist version; the former Taskless one is kept here
  "mention": [c(12, 12, 4, [[20, 14]], { accent: true }), p("M16 12v1.5a2.5 2.5 0 0 0 5 0V12a9 9 0 1 0-3.5 7.1", [[16, 10]])],
  // menu: the set uses the Taskless version; the Ansamblist variant is kept here
  "menu": [p("M4 7h16M4 12h16M4 17h16")],
  // mic: the set uses the Taskless version; the Ansamblist variant is kept here
  "mic": [rc(9, 2.5, 6, 11.5, 3, [[15, 11], [63, 11]]), p("M5.5 12v.5a6.5 6.5 0 0 0 13 0V12", [[42, 14]], { accent: true }), p("M12 19v2.5"), p("M8.5 21.5h7")],
  // mic-off: the set uses the Taskless version; the Ansamblist variant is kept here
  ...
  // music: the set uses the Ansamblist version; the former Taskless one is kept here
  "music": [p("M9 18V5l12-2v13", [[45, 14]]), c(6, 18, 3, [[20, 16]]), c(18, 16, 3, [[20, 16]])],
  // screen-share: the set uses the Taskless version; the Ansamblist variant is kept here
  ...
  // send: the set uses the Taskless version; the Ansamblist variant is kept here
  "send": [p("M22 2 11 13", [[36, 24]]), p("M22 2l-7 20-4-9-9-4z", [[10, 8], [60, 8]])],
  // video-off: the set uses the Ansamblist version; the former Taskless one is kept here
